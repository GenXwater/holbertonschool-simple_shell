...
void print_path_directories(char *envp[])
{
    ...
    char *dir;    if (!path)
        return;    path = strdup(path);  /* Duplicate the PATH to avoid modifying the original */
    dir = strtok(path, ":");    while (dir)
    {
        printf("%s\n", dir);
        dir = strtok(NULL, ":");
...
list_t *build_path_list(char *envp[])
{
    char *path = _getenv("PATH", envp);
    char *dir;
    list_t *head = NULL, *new_node, *last;    if (!path)
        return (NULL);    path = strdup(path);  /* Copy the PATH to avoid modifying the original */
    dir = strtok(path, ":");    while (dir)
    {
        new_node = malloc(sizeof(list_t));
        if (!new_node)
            return (NULL);        new_node->dir = strdup(dir);
        new_node->next = NULL;        if (!head)
            head = new_node;
        else
